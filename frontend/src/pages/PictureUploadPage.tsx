...
export default function PictureUploadPage({ phoneNumber, userId, email, name, gender }: PictureUploadPageProps): JSX.Element {
  ...
  void phoneNumber; void email; void gender; void userId; void name;
  ...
  useEffect(() => {
    const updateJourneyState = async () => {
      if (socket) {
        try {
          // Emit a request to update journey state to PICTURE_UPLOAD
          socket.emit('update_journey_state', { journeyState: 'PICTURE_UPLOAD' });
          console.log('🎯 Updated journey state to PICTURE_UPLOAD');
        } catch (error) {
      ...
      <main className="flex flex-col items-center justify-between w-full min-h-[calc(100vh-88px)] py-8 px-4">
        ...
        <div className="flex justify-center w-full mb-12">
          <button
            ...
            className="bg-white rounded-[30px] w-[123px] h-[123px] transition-all duration-300 hover:scale-105 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed relative"
          >
            {/* Dashed border */}
            <div className="absolute border-4 border-[#faaf5c] border-dashed inset-0 pointer-events-none rounded-[30px]" />
        ...
        <div className="flex justify-center w-full mb-10">
          <button
            ...
            className="min-w-[250px] rounded-[20px] border-2 border-white bg-transparent transition-all duration-300 hover:scale-105 hover:bg-white hover:text-purple-700 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <div className="flex items-center justify-center gap-2.5 p-[10px]">
